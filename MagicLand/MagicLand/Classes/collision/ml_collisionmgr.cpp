...
void MLCollisionMgr::RemoveColEntry(MLEntity* entity) {
  ...
  if (entity != NULL) {
		...
		ML_SAFE_ASSERT(mainType <= m_ColMgrTable.size(), "Doesn't have the specific type of the entity");
		if (mainType <= m_ColMgrTable.size()) {
			bool bFound = false;
...
void MLCollisionMgr::CollisionResponse() {
	for (int i = 0; i < m_ColMgrTable.size(); i++) {
		for (MLColMgrListIt it = m_ColMgrTable[i].begin(); it != m_ColMgrTable[i].end(); ++it) {
			...
			if (entity != NULL) {
				MLComState* pState = static_cast<MLComState*>(entity->GetComponent(ML_COMTYPE_STATE));
...
void MLCollisionMgr::ClearCollisionInfo() {
	for (int i = 0; i < m_ColMgrTable.size(); i++) {
		for (MLColMgrListIt it = m_ColMgrTable[i].begin(); it != m_ColMgrTable[i].end(); ++it) {
			...
			if (entity != NULL) {
				MLComBoundBox* pBox = static_cast<MLComBoundBox*>(entity->GetComponent(ML_COMTYPE_BOUNDBOX));
...
void MLCollisionMgr::DetectColPlayerWithEnv() {
	for (MLColMgrListIt itPlayer = m_ColMgrTable[ML_ETYMAINTYPE_PLAYER].begin();
		   itPlayer != m_ColMgrTable[ML_ETYMAINTYPE_PLAYER].end();
		   ++itPlayer) {
		MLEntity* pPlayer = *itPlayer;
		ML_SAFE_ASSERT(pPlayer != NULL, "There is an error");

		if (pPlayer != NULL) {
			MLComBoundBox* playerBox = static_cast<MLComBoundBox*>(pPlayer->GetComponent(ML_COMTYPE_BOUNDBOX));
			ML_SAFE_ASSERT(playerBox != NULL, "The entity don't have the entity");
			
      if(playerBox != NULL) {
				for (MLColMgrListIt itEnv = m_ColMgrTable[ML_ETYMAINTYPE_ENV].begin();
					   itEnv != m_ColMgrTable[ML_ETYMAINTYPE_ENV].end();
					   ++itEnv) {
					MLEntity* pEnv = *itEnv;
					ML_SAFE_ASSERT(pEnv != NULL, "There is an error");
					
          if(pEnv != NULL) {
						MLComBoundBox* envBox = static_cast<MLComBoundBox*>(pEnv->GetComponent(ML_COMTYPE_BOUNDBOX));
...
void MLCollisionMgr::DetectColEnemyWithMagic() {
	...
	for (MLColMgrListIt itEnemy = m_ColMgrTable[ML_ETYMAINTYPE_ENEMY].begin();
		   itEnemy != m_ColMgrTable[ML_ETYMAINTYPE_ENEMY].end();
		   ++itEnemy) {
    ...
    if (pEnemy != NULL) {
		  MLComBoundBox* enemyBox = static_cast<MLComBoundBox*>(pEnemy->GetComponent(ML_COMTYPE_BOUNDBOX));
		  ML_SAFE_ASSERT(enemyBox != NULL, "The entity don't have the entity");
		  
      if (enemyBox != NULL) {
			  for (MLColMgrListIt itMagic = m_ColMgrTable[ML_ETYMAINTYPE_PLAYER_MAGIC].begin();
             itMagic != m_ColMgrTable[ML_ETYMAINTYPE_PLAYER_MAGIC].end();
             ++itMagic) {

				  MLEntity* pMagic = *itMagic;
				  ML_SAFE_ASSERT(pMagic != NULL, "There is an error");
					
				  MLComBoundBox* magicBox = static_cast<MLComBoundBox*>(pMagic->GetComponent(ML_COMTYPE_BOUNDBOX));
...
void MLCollisionMgr::DetectColEnemyMagicWithEnv() {
	...
	for (MLColMgrListIt itEnemyMagic = m_ColMgrTable[ML_ETYMAINTYPE_ENEMYMAGIC].begin();
       itEnemyMagic != m_ColMgrTable[ML_ETYMAINTYPE_ENEMYMAGIC].end();
       ++itEnemyMagic) {
		MLEntity* pEnemyMagic = *itEnemyMagic;
		ML_SAFE_ASSERT(pEnemyMagic != NULL, "There is an error");

		MLComBoundBox* enemyMagicBox = static_cast<MLComBoundBox*>(pEnemyMagic->GetComponent(ML_COMTYPE_BOUNDBOX));
